...
```{r install-ggplot2, warning=FALSE, message=FALSE, include=FALSE, eval=TRUE}
...
Some species may not be found in the IUCN database due to taxonomic revisions, synonymy issues, or lack of formal assessment. Species without IUCN status `r paste0("(n =", length(not_found_sp), "):")` `r paste0("*", paste(names(unlist(not_found_sp)), collapse = ", "), "*")`.
...
```{r status-summary, eval=TRUE, include=FALSE}
